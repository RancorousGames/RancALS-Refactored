...
void UAlsAnimationInstance::NativeInitializeAnimation()
{
	...
	IsDedicatedServer = Character ? Character->GetNetMode() == NM_DedicatedServer : false;
#if WITH_EDITOR
...
void UAlsAnimationInstance::NativeUpdateAnimation(const float DeltaTime)
{
	...
	LockAimDirection = Character->GetLockAimDirectionWT();
	...
	// Wartribes mod
	Character->SetGlobalCharacterYawAndPitchWT(SpineState.YawAngle + LocomotionState.Rotation.Yaw, SpineState.PitchAngle);
}
...
void UAlsAnimationInstance::NativePostUpdateAnimation()
{
	...
	if (!IsValid(Settings) || !IsValid(Character) || IsDedicatedServer)
	{
...
void UAlsAnimationInstance::RefreshSpine(const float SpineBlendAmount, const float DeltaTime)
{
	...
	if (UseSimpleWTRotation)
	{
		...
		bool SpineRotationAllowed = IsSpineRotationAllowed(); // Could be unified with LockAimDirection but it gives some jitter when attacking that im too lazy to debug
		if (SpineRotationAllowed && !LockAimDirection)
		{
			if (!SpineState.bSpineRotationAllowed) // If we are switching from not allowed to allowed, reset the spine state
			{
				SpineState.YawAngle = 0;
				SpineState.LastActorYawAngle = LocomotionState.Rotation.Yaw;
				SpineState.YawSpeed = 0;
				SpineState.YawMoveTime = 0;
			}
			...
			float OldSpineYaw = SpineState.YawAngle;
			float BaseSpeed = Settings->WTCustom.SpineRotationBaseSpeed;
			float AccelerationMultiplier = FMath::Clamp(SpineState.YawMoveTime / Settings->WTCustom.SpineRotationAccelerationTimeToFullSpeed, Settings->WTCustom.SpineRotationAccelerationMinimumMultiplier, 1.f);
			SpineState.YawAngle = FMath::Clamp(
				UAlsRotation::LerpAngle(SpineState.YawAngle, ViewState.YawAngle, Settings->WTCustom.SpineRotationLerpDistFactor * DeltaTime * AccelerationMultiplier
					, BaseSpeed, ForcedRotationDirection),
				-90.f, 90.f
			) - ActorRotationDelta;
			SpineState.YawSpeed = FMath::Abs(FMath::FindDeltaAngleDegrees(OldSpineYaw, SpineState.YawAngle) + ActorRotationDelta) / DeltaTime;
			SpineState.YawMoveTime = SpineDeltaAngle > 0.1 ? SpineState.YawMoveTime + DeltaTime : 0;
			//UE_LOG(LogTemp, Warning, TEXT("ASDS MoveTime: %f, SpineState.YawSpeed: %f AAccelerationMultiplier: %f"), SpineState.YawMoveTime, SpineState.YawSpeed, AccelerationMultiplier);
			SpineState.LastActorYawAngle = UE_REAL_TO_FLOAT(LocomotionState.Rotation.Yaw);

			// Log all the varaibles used here, SpineDeltaAngle, bTurnLeft, ForcedRotationDirection, ActorRotationDelta, SpineState.YawAngle, ViewState.YawAngle, SpineState.LastActorYawAngle
			//UE_LOG(LogTemp, Warning, TEXT("SpineDeltaAngle: %f, bTurnLeft: %d, ForcedRotationDirection: %d, ActorRotationDelta: %f, SpineState.YawAngle: %f, ViewState.YawAngle: %f, SpineState.LastActorYawAngle: %f, DeltaTime: %f"), SpineDeltaAngle, bTurnLeft, ForcedRotationDirection, ActorRotationDelta, SpineState.YawAngle, ViewState.YawAngle, SpineState.LastActorYawAngle, DeltaTime);
		}
		else if (SpineState.YawAngle > 0 && !LockAimDirection)
		{
...
{
	if (!IsValid(Settings) || IsDedicatedServer)
	{
		return;
	}
...
void UAlsAnimationInstance::RefreshDynamicTransitions()
{
	...
	if (DynamicTransitionsState.bUpdatedThisFrame || !IsValid(Settings) || IsDedicatedServer)
	{
		return;
	}
...
void UAlsAnimationInstance::PlayQueuedTransitionAnimation()
{
	...
	if (TransitionsState.bStopTransitionsQueued || !IsValid(TransitionsState.QueuedTransitionSequence) || IsDedicatedServer)
	{
		return;
	}
...
{
	return (RotationMode == AlsRotationModeTags::Aiming && !LockAimDirection) || ViewMode == AlsViewModeTags::FirstPerson;
}
...
{
#if WITH_EDITOR
	if (!IsValid(GetWorld()) || !GetWorld()->IsGameWorld() || IsDedicatedServer)
	{
		return;
	}
#endif
...
void UAlsAnimationInstance::PlayQueuedTurnInPlaceAnimation()
{
	...
	if (TransitionsState.bStopTransitionsQueued || !IsValid(TurnInPlaceState.QueuedSettings) || IsDedicatedServer)
	{
		return;
	}
